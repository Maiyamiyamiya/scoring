...
function Start() {
  ...
  const saveDataSlugAka = async () => {
    setDataAkaState(dataAtlet);
      ...
      const newDocRef = push(ref(db, "score/tatami2/slugAka"));
      set(newDocRef, {
        slug: slugAka
        }).then( () => {
          setInputValue1("");
     ...
     const saveDataSlugAo = async () => {
      ...
      const db = getDatabase(app);
      const newDocRef = push(ref(db, "score/tatami2/slugAo"));
      set(newDocRef, {
        slug: slugAo
        }).then( () => {
          setInputValue1("");
    ...
    const deleteSlug = async (slugId) => {
      const db = getDatabase(app);
      const dbRefAka = ref(db, "score/tatami2/slugAka");
      const dbRefAo = ref(db, "score/tatami2/slugAo");
  ...
  const fetchDataSlugAo = async () => {
    ...
    const fetchDataAKA = async () => {
          ...
          if (dataSlugAka.length === 0) {
            ...
            const db = getDatabase(app);
            const dbRef = ref(db, `score/tatami2/atlet=${dataSlugAka[0].slug}`)
            const snapshot = await get(dbRef);
            if(snapshot.exists()) {
  ...
  const fetchDataAO = async () => {
      ...
      if (dataSlugAo.length === 0) {
        ...
        const db = getDatabase(app);
        const dbRef = ref(db, `score/tatami2/atlet=${dataSlugAo[0].slug}`); 
        const snapshot = await get(dbRef);
        if(snapshot.exists()) {
